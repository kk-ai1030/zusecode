"""从 .zusecode/commands/ 目录加载自定义 Markdown 命令。
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.zusecode/commands/
      2. <work_dir>/.zusecode/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".zusecode" / "commands"))
    dirs.append(str(Path(work_dir) / ".zusecode" / "commands"))
