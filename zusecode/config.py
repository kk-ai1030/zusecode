...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".zusecode" / "config.yaml",
        cwd / ".zusecode" / "config.yaml",
        cwd / ".zusecode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .zusecode/config.yaml "
            "in project or ~/.zusecode/config.yaml"
        )
    return merged
