...
class AgentTool(Tool):
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from zusecode.agents.fork import ForkError, build_forked_messages
        from zusecode.agents.parser import AgentDef
        from zusecode.agents.tool_filter import clone_registry_for_fork, resolve_agent_tools
        from zusecode.agent import Agent as AgentClass
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        from zusecode.agents.fork import ForkError, build_forked_messages
        from zusecode.agents.parser import AgentDef
        from zusecode.agents.tool_filter import build_teammate_tools
        from zusecode.agent import Agent as AgentClass
        from zusecode.conversation import ConversationManager
        from zusecode.permissions import (
            DangerousCommandDetector,
        ...
        from zusecode.teams.models import BackendType, TeammateInfo
    ...
    def _spawn_pane_teammate(
        ...
        from zusecode.teams.spawn import build_teammate_cli
        ...
        if mailbox is not None and p.prompt:
            from zusecode.teams.mailbox import create_message
            from zusecode.teams.spawn_inprocess import LEAD_NAME
            mailbox.write(
                teammate_name,
                create_message(
        ...
        # 构造把本 zusecode 拉起为队友 worker 模式的命令，cd 到该队友的 worktree
        cli_command = build_teammate_cli(p.team_name, teammate_name, wt.path)

        try:
            if backend == BackendType.TMUX:
                from zusecode.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from zusecode.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from zusecode.worktree.integration import (
    ...
    def _create_client_for_model(self, model_alias: str) -> LLMClient | None:
        ...
        from zusecode.client import create_client
