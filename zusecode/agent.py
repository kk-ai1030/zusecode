...
from zusecode.client import LLMClient
from zusecode.context import (
    CompactBoundary,
...
from zusecode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from zusecode.conversation_pairing import REJECTED_TOOL_RESULT
from zusecode.conversation import ThinkingBlock as ConvThinkingBlock
from zusecode.memory.auto_memory import MemoryManager
from zusecode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from zusecode.hooks import HookContext, HookEngine, ToolRejectedError
from zusecode.hooks.engine import HookNotification
from zusecode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
...
class Agent:
    def __init__(
        ...
        if memory_manager is not None:
            from zusecode.memory.consolidation import MemoryConsolidator
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".zusecode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".zusecode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from zusecode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(
        ...
        from zusecode.context.manager import (
