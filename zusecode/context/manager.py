...
from zusecode.serialization import build_messages
...
def spill_dir(work_dir: str, session_id: str = "") -> Path:
    """溢写目录：按会话隔离在 .zusecode/sessions/<会话id>/tool-results 下，
    会话 id 为空（一次性调用、测试）时落到 default。"""
    sid = session_id or "default"
    return Path(work_dir) / ".zusecode" / "sessions" / sid / "tool-results"
...
async def auto_compact(
    ...
    for attempt in range(max_retries):
        try:
            from zusecode.tools.base import StreamEnd, StreamEvent, TextDelta
