...
def _session_path(zusecode_dir: Path) -> Path:
    return zusecode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(zusecode_dir)
    if session is None:
...
def load_worktree_session(zusecode_dir: Path) -> WorktreeSession | None:
    path = _session_path(zusecode_dir)
    if not path.exists():
        return None
    try:
