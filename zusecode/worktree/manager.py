...
from zusecode.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from zusecode.worktree.models import Worktree, WorktreeSession
from zusecode.worktree.session import load_worktree_session, save_worktree_session
from zusecode.worktree.setup import perform_post_creation_setup
from zusecode.worktree.slug import flatten_slug, validate_slug
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".zusecode" / "worktrees"
        )
        self._zusecode_dir = Path(repo_root) / ".zusecode"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._zusecode_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._zusecode_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._zusecode_dir)
        if session is None:
            return None
