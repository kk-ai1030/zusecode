...
from zusecode.config import ConfigError, load_config
from zusecode.hooks import HookConfigError, HookEngine, load_hooks
from zusecode.permissions import PermissionMode


def main() -> None:
    # 队友 worker 模式：由 tmux/iTerm2 窗格用 `-m zusecode --teammate ...` 拉起。
    ...
    # 先确保 .zusecode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".zusecode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".zusecode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="zusecode", description="ZuseCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from zusecode.remote import RemoteServer
    ...
    from zusecode.app import ZuseCodeApp
    from zusecode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from zusecode.client import create_client, resolve_context_window
    from zusecode.conversation import ConversationManager
    from zusecode.memory.instructions import load_instructions
    ...
    from zusecode.tools import create_default_registry
    ...
    from zusecode.worktree import WorktreeManager
...
async def _build_teammate_registry(
    ...
    from zusecode.tools.synthetic_output import SyntheticOutputTool
    from zusecode.tools.task_create import TaskCreateTool
    from zusecode.tools.task_get import TaskGetTool
    from zusecode.tools.task_list import TaskListTool
    from zusecode.tools.task_update import TaskUpdateTool
    from zusecode.worktree import WorktreeManager
...
async def _run_teammate(team_name: str, agent_name: str) -> None:
    ...
    """
    from zusecode.agent import Agent
    from zusecode.client import create_client, resolve_context_window
    from zusecode.memory.instructions import load_instructions
    from zusecode.permissions import (
        DangerousCommandDetector,
    ...
    from zusecode.teams.spawn_inprocess import LEAD_NAME, spawn_inprocess_teammate
