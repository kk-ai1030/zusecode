...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.zusecode/ZUSECODE.md, ~/.zusecode/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 ZUSECODE.md 和 AGENTS.md
    3. workDir/.zusecode/INSTRUCTIONS.md（遗留格式）
    4. workDir/ZUSECODE.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".zusecode" / "ZUSECODE.md")
    _add(home / ".zusecode" / "AGENTS.md")
    ...
    # 3. 遗留格式
    _add(root / ".zusecode" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "ZUSECODE.local.md")
