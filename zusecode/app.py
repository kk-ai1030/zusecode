...
from zusecode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from zusecode.commands.completion import CompletionPopup
from zusecode.commands.handlers import register_all_commands
from zusecode.config import MCPServerConfig, ProviderConfig
from zusecode.hooks import HookContext, HookEngine, load_hooks
from zusecode.conversation import ConversationManager, Message
from zusecode.mcp import ConnectResult, MCPManager
from zusecode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from zusecode.permissions import (
...
from zusecode.agents.task_manager import TaskManager
from zusecode.agents.trace import TraceManager
from zusecode.agents.notification import inject_task_notifications
from zusecode.commands.handlers.tasks import create_tasks_command
from zusecode.skills.executor import SkillExecutor
from zusecode.skills.loader import SkillLoader
from zusecode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from zusecode.tools import ToolRegistry, create_default_registry
from zusecode.tools.agent_tool import AgentTool
from zusecode.tools.ask_user import AskUserEvent, AskUserTool
...
from zusecode.worktree.cleanup import start_stale_cleanup_task
from zusecode.worktree.manager import WorktreeManager
from zusecode.commands.handlers.worktree import create_worktree_command
from zusecode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".zusecode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".zusecode" / "history"
...
class ZuseCodeApp(App):
    ...
    def __init__(
        ...
        from zusecode.config import SandboxAppConfig
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        t = RichText()
        t.append("\n", style="color(242)")
        t.append("  ___   _  _   ___   ___   ", style="bold color(99)")
        t.append("ZuseCode v0.1.0\n", style="color(242)")
        t.append(" |_ /  | || |  (_-<  / -_)  ", style="bold color(99)")
        t.append(f"{model}\n" if model else "\n", style="color(242)")
        t.append(r" /__|   \_,_|  /__/  \___|  ", style="bold color(99)")
    ...
    def on_mount(self) -> None:
        self.register_theme(_ZUSECODE_THEME)
        self.theme = "zusecode"
        if len(self.providers) == 1:
            ...
            # 必须显式聚焦列表：Textual 会把初始焦点落到更靠后的 chat-area
            # （VerticalScroll 默认可聚焦），导致上下键/回车全被聊天区吞掉，
            # provider 列表拿不到焦点就无法选择。
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".zusecode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".zusecode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".zusecode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from zusecode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.zusecode/config.yaml",
                        f"{work_dir}/.zusecode/permissions.local.yaml",
                    ],
        ...
        from zusecode.filehistory import FileHistory
        ...
        # --- Worktree 系统初始化 ---
        from zusecode.config import WorktreeConfig
        ...
        from zusecode.tools.team_delete import TeamDeleteTool
        ...
        from zusecode.commands.handlers.trace import create_trace_command
        ...
        from zusecode.tools.synthetic_output import SyntheticOutputTool
        from zusecode.tools.task_stop import TaskStopTool
        ...
        if self._enable_coordinator_mode:
            from zusecode.agents.tool_filter import apply_coordinator_filter
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from zusecode.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from zusecode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from zusecode.plan_dialog import InlinePlanWidget, PlanChoice
        from zusecode.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from zusecode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from zusecode.permission_dialog import InlinePermissionWidget
