...
from zusecode.teams.backend_detect import (
    BackendDetectionError,
    detect_backend,
    detect_backend_from_env,
    detect_pane_backend,
)
from zusecode.teams.coordinator import (
    get_coordinator_system_prompt,
    get_coordinator_user_context,
)
from zusecode.agents.tool_filter import (
...
class TestModels:
    ...
    def test_unique_team_name(self, tmp_dir):
        with patch("zusecode.teams.models.Path.home", return_value=Path(tmp_dir)):
            name1 = unique_team_name("my-team")
            assert name1 == "my-team"
            (Path(tmp_dir) / ".zusecode" / "teams" / "my-team").mkdir(parents=True)
...
class TestBackendDetect:
    ...
    def test_detect_backend_windows_always_in_process(self):
        ...
        env["TMUX"] = "/tmp/tmux-1234/default,12345,0"
        with patch.dict(os.environ, env, clear=True):
            with patch("zusecode.teams.backend_detect.sys.platform", "win32"):
    ...
    def test_detect_backend_posix_tmux(self):
        ...
        env["TMUX"] = "/tmp/tmux-1234/default,12345,0"
        with patch.dict(os.environ, env, clear=True):
            with patch("zusecode.teams.backend_detect.sys.platform", "linux"):
    ...
    def test_pane_backend_posix_iterm(self):
        ...
        env["ITERM_SESSION_ID"] = "w0t0p0:ABC-123"
        with patch.dict(os.environ, env, clear=True):
            with patch("zusecode.teams.backend_detect.sys.platform", "darwin"):
...
class TestCoordinatorMode:
    ...
    def test_system_prompt_uses_real_subagent_type(self):
        # ZuseCode 的内建类型是 general-purpose / plan / explore，没有 worker，
...
class TestConfigExtensions:
    def test_teammate_mode_defaults(self):
        from zusecode.config import AppConfig
    ...
    def test_load_config_with_team_fields(self, tmp_dir):
        from zusecode.config import load_config
...
class TestTranscript:
    ...
    def test_save_and_load(self, tmp_dir):
        ...
        from zusecode.teams.transcript import load_transcript, save_transcript
    ...
    def test_load_nonexistent(self, tmp_dir):
        from zusecode.teams.transcript import load_transcript
        with patch("zusecode.teams.models.Path.home", return_value=Path(tmp_dir)):
...
class TestAgentCoordinatorIntegration:
    def test_normal_prompt(self):
        from zusecode.prompts import build_system_prompt, IDENTITY_SECTION
        prompt = build_system_prompt()
        # 验证 identity section 内容包含在 prompt 中
        assert "ZuseCode" in prompt
    ...
    def test_coordinator_guidance_is_a_reminder_not_a_replacement(self):
        ...
        from zusecode.prompts import build_system_prompt, IDENTITY_SECTION
        from zusecode.teams.coordinator import get_coordinator_system_prompt
    ...
    def test_coordinator_reminder_lists_only_allowed_tools(self):
        from zusecode.agents.tool_filter import COORDINATOR_MODE_ALLOWED_TOOLS
        from zusecode.teams.coordinator import get_coordinator_system_prompt
...
class TestTaskStopTool:
    ...
    async def test_stops_in_process_teammate(self):
        from zusecode.tools.task_stop import TaskStopTool, TaskStopParams
...
class TestCoordinatorReminderShape:
    ...
    def test_reminder_goes_sparse_after_first_turn(self):
        from zusecode.teams.coordinator import get_coordinator_reminder
