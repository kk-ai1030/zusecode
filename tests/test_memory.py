...
from zusecode.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    ToolUseBlock,
...
from zusecode.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
from zusecode.memory.session import (
    ResumeResult,
    Session,
    SessionManager,
    SessionMeta,
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        zusecode_md = tmp_path / "ZUSECODE.md"
        zusecode_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """发现顺序：ZUSECODE.md 在前，.zusecode/INSTRUCTIONS.md（legacy）在后。"""
        root_md = tmp_path / "ZUSECODE.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".zusecode"
        dotdir.mkdir()
        # 不发现 .zusecode/ZUSECODE.md，只发现 .zusecode/INSTRUCTIONS.md（legacy）
...
class TestSession:
    def test_append_writes_jsonl_and_updates_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".zusecode" / "sessions"
...
class TestMemoryManager:
    ...
    def test_load_includes_memory_index(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 创建项目级记忆目录和文件
        project_mem_dir = tmp_path / "project" / ".zusecode" / "memory"
    ...
    def test_load_all(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 用户级记忆
        user_mem_dir = fake_home / ".zusecode" / "memory"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "zusecodeMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "zusecodeMd" in conv.history[0].content
...
class TestMemoryExtraction:
    def test_memory_types_aligned_with_go(self, tmp_path: Path) -> None:
        """验证四种记忆类型枚举。"""
        from zusecode.memory.auto_memory import VALID_TYPES, _USER_LEVEL_TYPES, _PROJECT_LEVEL_TYPES
