...
需要环境变量 ZUSECODE_TEST_API_KEY、ZUSECODE_TEST_BASE_URL、ZUSECODE_TEST_MODEL。
...
def test_lock_first_acquire():
    from zusecode.memory.consolidation import _read_last_consolidated_at, _try_acquire_lock
...
def test_lock_blocks_when_held():
    from zusecode.memory.consolidation import _try_acquire_lock
...
def test_lock_rollback_deletes_on_zero():
    from zusecode.memory.consolidation import _try_acquire_lock, _rollback_lock
...
def test_prompt_contains_all_phases():
    from zusecode.memory.consolidation import _build_consolidation_prompt
...
@pytest.mark.skipif(
    not os.environ.get("ZUSECODE_TEST_API_KEY"),
    reason="ZUSECODE_TEST_API_KEY not set"
)
@pytest.mark.timeout(120)
def test_e2e_consolidation_merges_duplicates():
    api_key = os.environ["ZUSECODE_TEST_API_KEY"]
    base_url = os.environ.get("ZUSECODE_TEST_BASE_URL", "https://api.minimaxi.com/v1")
    model = os.environ.get("ZUSECODE_TEST_MODEL", "MiniMax-M3")

    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".zusecode", "memory")
...
async def _run_consolidation(work_dir, api_key, base_url, model, mem_dir):
    ...
    from zusecode.permissions.checker import PermissionChecker
    from zusecode.tools import ToolRegistry
    from zusecode.tools.bash import Bash
    from zusecode.tools.edit_file import EditFile
    from zusecode.tools.glob import Glob
    from zusecode.tools.grep import Grep
    from zusecode.tools.read_file import ReadFile
    from zusecode.tools.write_file import WriteFile

    from zusecode.config import ProviderConfig
    from zusecode.client import OpenAICompatClient
    ...
    from zusecode.permissions.rules import RuleEngine
    from zusecode.permissions.dangerous import DangerousCommandDetector
    from zusecode.permissions.checker import PermissionMode
